...
function systemPrompt(analyses: Analysis[], options: Record<string, unknown>, instructions?: string): string {
  ...
  return [
    ...
    '- juntos: agrupados | individual · orientacion: vertical | horizontal · sinMargenes: true|false · ladoEncuadernacion: largo | corto',
    '',
    ...
    '- ORIENTACIÓN: fija "orientacion" según el campo "orientation" detectado del documento. Y para la encuadernación, lo habitual: si es VERTICAL, ladoEncuadernacion="largo"; si es HORIZONTAL, ladoEncuadernacion="corto" (puedes ajustarlo si el contexto lo pide).',
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (await llmRateLimited(req)) return res.status(429).json({ error: 'Has hecho muchas peticiones de sugerencia. Espera unos minutos.' });
